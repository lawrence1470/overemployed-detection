...
export async function GET(request: Request) {
  ...
  if (env.NODE_ENV === "production" && debug !== "true") {
    return NextResponse.json({ error: "Not available in production without debug=true" }, { status: 403 });
  }
  ...
  return NextResponse.json({
    ...
    all_env_keys: Object.keys(process.env).filter(key => key.includes("COMING")),
    comparison: {
