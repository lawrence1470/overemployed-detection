...
export const designSystem = {
	...
	components: {
		...
		button: {
			primary:
				"rounded-xl bg-gradient-to-r from-white to-gray-100 text-black font-semibold shadow-lg hover:shadow-xl transition-all duration-300",
			secondary:
				"rounded-xl border border-white/20 bg-white/10 text-white font-medium backdrop-blur-xl hover:bg-white/20 transition-all duration-300",
			ghost:
				"rounded-xl text-white/80 hover:text-white hover:bg-white/10 transition-all duration-300",
		},

		// Badges
		badge: {
			primary:
				"inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-white/10 text-white border border-white/20 backdrop-blur-xl",
			accent:
...
export const getComponentClasses = {
	section: () =>
		`${designSystem.layouts.section} ${designSystem.spacing.section}`,
